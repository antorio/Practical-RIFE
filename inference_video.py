...
class FFmpegPipeWriter:
    ...
    to ffmpeg, giving proper H.264/H.265 encoding with CRF quality control.
    Same interface: .write(bgr_frame), .isOpened(), .release()
    """
    ...
    def write(self, frame):
        try:
            ...
            raise RuntimeError(
                f"ffmpeg pipe closed unexpectedly while writing to {self.path}"
            )
...
parser.add_argument("--crf", dest="crf", type=int, default=18,
                    help="CRF quality for ffmpeg (lower = better quality, 18 is near-lossless for H.264)")
parser.add_argument("--codec", dest="codec", type=str, default="libx264",
                    help="video codec for ffmpeg, e.g. libx264, libx265, libsvtav1")
parser.add_argument("--preset", dest="preset", type=str, default="slow",
                    help="ffmpeg encoding preset, e.g. ultrafast, fast, medium, slow, veryslow")
...
if args.png:
    ...
    vid_out = FFmpegPipeWriter(vid_out_name, args.fps, w, h, crf=args.crf, codec=args.codec, preset=args.preset)
